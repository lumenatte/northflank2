...
const UUID = process.env.UUID || '5c091692-cd73-4d02-8d8a-cb3ff76dbe21'; // 节点UUID,在不同的平台运行需修改UUID,否则会覆盖
const KOMARI_ENDPOINT = process.env.KOMARI_ENDPOINT || ''; // komari面板地址,例如: https://komari.abc.com
const KOMARI_TOKEN = process.env.KOMARI_TOKEN || '';       // komari探针的token
...
async function downloadFilesAndRun() {
  ...
  const filesToAuthorize = (KOMARI_ENDPOINT && KOMARI_TOKEN) ? [komariPath, webPath, botPath] : [webPath, botPath];
  authorizeFiles(filesToAuthorize);

  // 运行komari探针
  if (KOMARI_ENDPOINT && KOMARI_TOKEN) {
    const command = `nohup ${komariPath} -e ${KOMARI_ENDPOINT} -t ${KOMARI_TOKEN} --disable-web-ssh --disable-auto-update >/dev/null 2>&1 &`;
    try {
      await exec(command);
      console.log(`${komariName} is running`);
      await new Promise((resolve) => setTimeout(resolve, 1000));
    } catch (error) {
      console.error(`komari running error: ${error}`);
    }
  } else {
    console.log('KOMARI_ENDPOINT or KOMARI_TOKEN is empty, skip running');
  }
